...
## Dành cho ứng viên giải thích với Nhà tuyển dụng (NTD)

Khi chạy file `run_all_tests.bat`, bạn đã thấy giao diện Console chạy các dòng log và thông báo `24 passed in 56.58s`. Dưới đây là cách giải thích chuyên nghiệp cho NTD:
...
Đó chính là sức mạnh của **UI Automation Testing (Kiểm thử tự động giao diện)**.
...
- **Tự gõ**: Framework áp dụng mô hình **POM (Page Object Model)** để xác định vị trí của thẻ HTML (như ô Input Username/Password) thông qua ID/XPath. Sau đó dùng lệnh `send_keys` (với tốc độ `time.sleep` mà tôi đã set chậm lại để giống con người) để truyền text vào.
...
Bạn sẽ thấy kịch bản `test_invalid_login` cố tình gõ sai User/Pass, hệ thống web báo lỗi đỏ, nhưng màn hình CMD vẫn in chữ màu xanh `PASSED`. Tại sao?
- **Trả lời NTD**: "Dạ vì đây là **Negative Testing** (Test kịch bản lỗi). Expectation (Kết quả mong đợi) của em là *Web PHẢI hiện ra dòng chữ lỗi Đỏ đó*. Trong code Automation, em đã lấy dòng chữ đỏ đó ra và so sánh: Nếu dòng chữ đó khớp 100% với file JSON data, tức là chức năng *Bắt lỗi đăng nhập của Web* hoạt động chính xác. Do đó Test Case này PASSED ạ."
- Tính năng này được gọi là **DDT (Data-Driven Testing)**: Em đã đọc danh sách các user sai từ file `test_data.json` và lặp lại logic test tự động.
...
## 🎯 CÂU HỎI THƯỜNG GẶP TỪ NTD & CÁCH TRẢ LỜI

### ❓ Q: "Em viết code Automation thì em dùng mô hình gì để dễ bảo trì?"
**Trả lời**: Dạ em dùng mô hình **POM (Page Object Model)** kết hợp **Data-Driven**. POM giúp em tách các Locator (Id, Class) của nút bấm ra khỏi code logic test. Nếu ngày mai website đổi giao diện, em chỉ cần vào file `pages/login_page.py` để sửa 1 đoạn nhỏ, mà KHÔNG cần đụng chạm gì đến hàng chục file Test Cases ở ngoài. Việc này giúp giảm chi phí maintain cực kỳ hiệu quả ạ.

### ❓ Q: "Tại sao em lại kết hợp song song giữa Unit Test, API Test và UI Test?"
**Trả lời**: Dạ vì em áp dụng mô hình **Test Pyramid (Kim tự tháp kiểm thử)**. Làm E2E Test (UI Test) thì quan sát rất trực quan nhưng chạy rất chậm và dễ đứt gãy (flaky). Do đó với những logic IF-ELSE tính toán như bài Test của công ty, em dùng Unit Test kết hợp kỹ thuật BVA (Giá trị biên) và EP (Phân lớp tương đương) để chạy nhanh gọn trong 0.1 giây. Với data, em test trực tiếp qua API (Postman/Requests) để đảm bảo Backend ổn định.

### ❓ Q: "Vậy bình thường e code xong thì ai sẽ bấm chạy cái này?"
**Trả lời**: Dạ em đã cấu hình sẵn một luồng **CI/CD** bằng file `.github/workflows`. Trong mô hình hiện đại, mỗi khi anh Dev bấm "Push code" lên server, Github Actions sẽ tự động dựng một máy ảo Ubuntu, tự bật Server ngầm (Headless chrome) chạy bộ đồ nghề này của em trong vòng 2 phút. Nếu code Pass 100% thì nó mới cho Dev merge. Như vậy em "Shift-Left" quy trình QA, tiết kiệm thời gian test manual lặp đi lặp lại rất nhiều ạ.
